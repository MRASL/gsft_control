...
lqr_fullModelClass gController;
...
void OdometryCallback(const nav_msgs::Odometry::ConstPtr &odom) {
  ...
  gController.lqr_full_U.X[0 ]  = odometry.position_W.x();
  gController.lqr_full_U.X[1 ]  = odometry.position_W.y();
  gController.lqr_full_U.X[2 ]  = odometry.position_W.z();

  Eigen::Matrix3d R_W_B = odometry.orientation_W_B.toRotationMatrix();
  Eigen::Vector3d velocity_W =  R_W_B * odometry.velocity_B;

  gController.lqr_full_U.X[3 ]  = velocity_W.x();
  gController.lqr_full_U.X[4 ]  = velocity_W.y();
  gController.lqr_full_U.X[5 ]  = velocity_W.z();
  ...
  gController.lqr_full_U.X[6 ]  = phi;
  gController.lqr_full_U.X[7 ]  = teta;
  gController.lqr_full_U.X[8 ]  = psi;
  ...
  gController.lqr_full_U.X[9 ]  = euler_rate.x();
  gController.lqr_full_U.X[10]  = euler_rate.y();
  gController.lqr_full_U.X[11]  = euler_rate.z();
}
...
void MultiDofJointTrajectoryCallback(
    ...
    gController.lqr_full_U.z_ref    = eigen_reference.position_W.z();
  ...
  while(ros::ok()) {
    if (gCommand_active) {
        ...
        for(unsigned int i=0; i< 6; i++) {
            motor_RPM[i]     = gController.lqr_full_Y.motor_RPM[i];
            motor_command[i] = gController.lqr_full_Y.motor_command[i];
            motor_speed[i]   = gController.lqr_full_Y.motor_speed[i];
        ...
        virtual_contrl_msg->total_thrust = gController.lqr_full_Y.virtual_control[0];

        Eigen::VectorXd moment(3);
        for(unsigned int i=0; i< 2; i++)
            moment[i] = gController.lqr_full_Y.virtual_control[i+1];
        moment[2] = 0.0;    // yaw
