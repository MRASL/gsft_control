...
void tuning_GS1ModelClass::step()
{
...

  {
     ...
     *  Sum: '<S10>/Sum1'
     */
    rtb_ref_idx_0 = ((tuning_GS1_U.gain[8] * tuning_GS1_X.Integrator1_CSTATE -
                      (tuning_GS1_U.gain[6] * rtb_d_z + tuning_GS1_U.gain[7] *
                       tuning_GS1_U.X[5])) * 0.64935064935064934 + 9.81) * 1.54;
    ...
    for (i = 0; i < 6; i++) {
       ...
       *  Saturate: '<Root>/                  '
       */
      u0 = tuning_GS1_ConstP.ControlAllocation_Gain[i + 18] * rtb_ref_idx_3 +
        (tuning_GS1_ConstP.ControlAllocation_Gain[i + 12] * rtb_ff_idx_1 +
         (tuning_GS1_ConstP.ControlAllocation_Gain[i + 6] * rtb_ff_idx_0 +
          tuning_GS1_ConstP.ControlAllocation_Gain[i] * rtb_ref_idx_0));
    ...
    for (i = 0; i < 6; i++) {
      ...
      if (u0 > 200.0) {
        ...
        tuning_GS1_Y.motor_command[i] = u0;
      }
    ...
    tuning_GS1_Y.virtual_control[0] = rtb_ref_idx_0;
    ...
    if (rtmIsMajorTimeStep((&tuning_GS1_M)) &&
      ...
      for (i = 0; i < 3; i++) {
        u0 = 0.0;
        for (i_0 = 0; i_0 < 6; i_0++) {
          u0 += tuning_GS1_ConstP.C_Value[3 * i_0 + i] *
            tuning_GS1_DW.MemoryX_DSTATE[i_0];
        }

        rtb_Sum1_l[i] = tuning_GS1_U.X[9 + i] - u0;
      }
      ...
      for (i = 0; i < 3; i++) {
        rtb_Sum3_i[i] = (((tuning_GS1_ConstP.Constant1_Value[i + 3] * rtb_Add_a
                          ...
                          6] * rtb_Add_a[5]) +
                         tuning_GS1_ConstP.Constant_Value_g[i]) + (rtb_Add_f[i]
          - rtb_Add_f_0[i]);
      }
    ...
    if (rtmIsMajorTimeStep((&tuning_GS1_M)) &&
      ...
      if (tuning_GS1_DW.MeasurementUpdate_MODE) {
        for (i = 0; i < 3; i++) {
          ...
          rtb_Sum1_l[i] = 0.0;
          for (i_0 = 0; i_0 < 6; i_0++) {
            rtb_Sum1_l[i] += tuning_GS1_ConstP.C_Value[3 * i_0 + i] *
              rtb_MemoryX[i_0];
          }
      ...
      for (i = 0; i < 6; i++) {
        ...
        rtb_LOE_out[i] = 0.0;
        for (i_0 = 0; i_0 < 6; i_0++) {
          rtb_LOE_out[i] += tuning_GS1_ConstP.A_Value[6 * i_0 + i] *
            rtb_MemoryX[i_0];
        }
        ...
        /* Sum: '<S33>/Add' */
        rtb_Add[i] = rtb_LOE_out[i] + tuning_GS1_B.Product3[i];
      }
    }
