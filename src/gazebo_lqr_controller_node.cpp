...
gazebo_lqrModelClass gController;
...
void OdometryCallback(const nav_msgs::Odometry::ConstPtr &odom) {
  ...
  gController.gazebo_lqr_U.X[0 ]  = odometry.position_W.x();
  gController.gazebo_lqr_U.X[1 ]  = odometry.position_W.y();
  gController.gazebo_lqr_U.X[2 ]  = odometry.position_W.z();

  Eigen::Matrix3d R_W_B = odometry.orientation_W_B.toRotationMatrix();
  Eigen::Vector3d velocity_W =  R_W_B * odometry.velocity_B;

  gController.gazebo_lqr_U.X[3 ]  = velocity_W.x();
  gController.gazebo_lqr_U.X[4 ]  = velocity_W.y();
  gController.gazebo_lqr_U.X[5 ]  = velocity_W.z();

/*  gController.gazebo_lqr_U.X[3 ]  = odometry.velocity_B.x();
  gController.gazebo_lqr_U.X[4 ]  = odometry.velocity_B.y();
  gController.gazebo_lqr_U.X[5 ]  = odometry.velocity_B.z(); */
  ...
  gController.gazebo_lqr_U.X[6 ]  = phi;
  gController.gazebo_lqr_U.X[7 ]  = teta;
  gController.gazebo_lqr_U.X[8 ]  = psi;
...
/*  Eigen::Matrix3d H;
  ...
  gController.gazebo_lqr_U.X[9 ]  = euler_rate.x();
  gController.gazebo_lqr_U.X[10]  = euler_rate.y();
  gController.gazebo_lqr_U.X[11]  = euler_rate.z();
*/
  gController.gazebo_lqr_U.X[9 ]  = odometry.angular_velocity_B.x();
  gController.gazebo_lqr_U.X[10]  = odometry.angular_velocity_B.y();
  gController.gazebo_lqr_U.X[11]  = odometry.angular_velocity_B.z();
...
/*void MultiDofJointTrajectoryCallback(
    ...
    gController.gazebo_lqr_U.z_ref    = eigen_reference.position_W.z();
...
/*void CommandPoseCallback(const geometry_msgs::PoseStampedConstPtr& pose_msg) {
  ...
  gController.gazebo_lqr_U.x_ref    = eigen_reference.position_W.x();
  gController.gazebo_lqr_U.y_ref    = eigen_reference.position_W.y();
  gController.gazebo_lqr_U.z_ref    = eigen_reference.position_W.z();
  gController.gazebo_lqr_U.psi_ref  = 0.0;
  if (!gCommand_active){
    gCommand_active      = true;
  }
}
...
int main(int argc, char** argv) {
  ros::init(argc, argv, "gazebo_lqr_controller_node");
  ros::NodeHandle nh;
  ros::NodeHandle pnh("~");
  ROS_INFO("gazebo_lqr_controller_node main started");
  ...
  while(ros::ok()) {
    if (gCommand_active) {
    /*    gController.gazebo_lqr_U.X0[0] = gInit_value[0];
        gController.gazebo_lqr_U.X0[1] = gInit_value[1];
        gController.gazebo_lqr_U.X0[2] = gInit_value[2];
        gController.gazebo_lqr_U.X0[3] = gInit_value[3]; */
        ...
        for(unsigned int i=0; i< 6; i++) {
            ...
            {
              motor_RPM[i]     = gController.gazebo_lqr_Y.motor_RPM[i]*(1.0 - gLOE[i]);
              motor_command[i] = gController.gazebo_lqr_Y.motor_command[i]*(1.0 - gLOE[i]);
              motor_speed[i]   = gController.gazebo_lqr_Y.motor_speed[i]*(1.0 - gLOE[i]);
            }
        }
    ...
    uav_state_msg->position_ref.x  = gController.gazebo_lqr_Y.ref[0];
    uav_state_msg->position_ref.y  = gController.gazebo_lqr_Y.ref[1];
    uav_state_msg->position_ref.z  = gController.gazebo_lqr_Y.ref[2];
    uav_state_msg->heading_ref     = gController.gazebo_lqr_Y.ref[3];

    uav_state_msg->position_W.x  = gController.gazebo_lqr_U.X[0];
    uav_state_msg->position_W.y  = gController.gazebo_lqr_U.X[1];
    uav_state_msg->position_W.z  = gController.gazebo_lqr_U.X[2];
    uav_state_msg->velocity_B.x  = gController.gazebo_lqr_U.X[3];
    uav_state_msg->velocity_B.y  = gController.gazebo_lqr_U.X[4];
    uav_state_msg->velocity_B.z  = gController.gazebo_lqr_U.X[5];
    uav_state_msg->euler_angle.x = gController.gazebo_lqr_U.X[6];
    uav_state_msg->euler_angle.y = gController.gazebo_lqr_U.X[7];
    uav_state_msg->euler_angle.z = gController.gazebo_lqr_U.X[8];
    uav_state_msg->rotation_speed_B.x  = gController.gazebo_lqr_U.X[9];
    uav_state_msg->rotation_speed_B.y  = gController.gazebo_lqr_U.X[10];
    uav_state_msg->rotation_speed_B.z  = gController.gazebo_lqr_U.X[11];
    uav_state_msg->total_thrust  = gController.gazebo_lqr_Y.virtual_control[0];
    uav_state_msg->moment.x      = gController.gazebo_lqr_Y.virtual_control[1];
    uav_state_msg->moment.y      = gController.gazebo_lqr_Y.virtual_control[2];
    uav_state_msg->moment.z      = gController.gazebo_lqr_Y.virtual_control[3];

    uav_state_msg->LOE13.x  = gController.gazebo_lqr_Y.gamma[0];
    uav_state_msg->LOE13.y  = gController.gazebo_lqr_Y.gamma[1];
    uav_state_msg->LOE13.z  = gController.gazebo_lqr_Y.gamma[2];
/*    uav_state_msg->LOE46.x  = gController.gazebo_lqr_Y.gamma[3];
    uav_state_msg->LOE46.y  = gController.gazebo_lqr_Y.gamma[4];
    uav_state_msg->LOE46.z  = gController.gazebo_lqr_Y.gamma[5]; */
    ...
    if (gEmergency_status)
    {
      ...
      ROS_ERROR("gazebo_lqr_controller_node Emergency status");
