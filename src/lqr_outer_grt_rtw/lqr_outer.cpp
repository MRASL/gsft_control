...
void lqr_outerModelClass::step()
{
   ...
   *  Inport: '<Root>/X'
   */
  memcpy(&tmp[0], &lqr_outer_U.X[0], sizeof(real_T) << 3U);
  tmp[8] = lqr_outer_U.X[9];
  tmp[9] = lqr_outer_U.X[10];
  for (i_0 = 0; i_0 < 3; i_0++) {
    ...
    tmp_0[i_0] = 0.0;
    for (i = 0; i < 10; i++) {
      tmp_0[i_0] += lqr_outer_ConstP._Gain_g[3 * i + i_0] * tmp[i];
    }
     ...
     *  Sum: '<Root>/Sum'
     */
    rtb_Sum2[i_0] = ((lqr_outer_ConstP._Gain[i_0 + 6] *
                      lqr_outer_X.Integrator_CSTATE[2] +
                      (lqr_outer_ConstP._Gain[i_0 + 3] *
                       lqr_outer_X.Integrator_CSTATE[1] +
                       lqr_outer_ConstP._Gain[i_0] *
                       lqr_outer_X.Integrator_CSTATE[0])) - tmp_0[i_0]) +
  ...
  for (i_0 = 0; i_0 < 6; i_0++) {
     ...
     *  Gain: '<Root>/Gain2'
     */
    rtb_Sqrt1 = std::sqrt(116978.4923343994 * (lqr_outer_ConstP.Gain_Gain[i_0 +
      12] * rtb_Sum2[2] + (lqr_outer_ConstP.Gain_Gain[i_0 + 6] * rtb_Sum2[1] +
      lqr_outer_ConstP.Gain_Gain[i_0] * rtb_Sum2[0])));
    ...
    lqr_outer_Y.motor_speed[i_0] = rtb_Sqrt1;

    /* Outport: '<Root>/motor_RPM' */
    lqr_outer_Y.motor_RPM[i_0] = rtb_rads_to_RPM;
  }
   ...
   *  Inport: '<Root>/z_ref'
   */
  lqr_outer_B.Sum1[0] = 0.0 - lqr_outer_U.X[0];
  lqr_outer_B.Sum1[1] = 0.0 - lqr_outer_U.X[1];
  lqr_outer_B.Sum1[2] = lqr_outer_U.z_ref - lqr_outer_U.X[2];
  if (rtmIsMajorTimeStep((&lqr_outer_M))) {
...
void lqr_outerModelClass::lqr_outer_derivatives()
{
  ...
  _rtXdot->Integrator_CSTATE[0] = lqr_outer_B.Sum1[0];
  _rtXdot->Integrator_CSTATE[1] = lqr_outer_B.Sum1[1];
  _rtXdot->Integrator_CSTATE[2] = lqr_outer_B.Sum1[2];
}
...
void lqr_outerModelClass::initialize()
{
  ...
  (&lqr_outer_M)->Timing.stepSize0 = 0.016666666666666666;
