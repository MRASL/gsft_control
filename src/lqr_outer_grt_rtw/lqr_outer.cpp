...
void lqr_outerModelClass::step()
{
  ...
  for (i = 0; i < 4; i++) {
     ...
     *  Sum: '<S1>/Sum'
     */
    rtb_rads_to_RPM = lqr_outer_ConstP._Gain[i + 12] *
     ...
     *  Sum: '<S1>/Sum'
     */
    rtb_TmpSignalConversionAtInpo_h[i] = (rtb_rads_to_RPM - tmp_0[i]) +
  ...
  for (i = 0; i < 6; i++) {
     ...
     *  Gain: '<Root>/Gain2'
     */
    rtb_rads_to_RPM = lqr_outer_ConstP.Gain_Gain[i + 18] *
     ...
     *  Gain: '<Root>/Gain2'
     */
    psi = std::sqrt(116978.4923343994 * rtb_rads_to_RPM);
    ...
    if (rtb_Clock > 200.0) {
      ...
      /* Outport: '<Root>/motor_command' */
      lqr_outer_Y.motor_command[i] = rtb_Clock;
    }

    /* End of Saturate: '<Root>/Saturation' */

    /* Outport: '<Root>/motor_speed' */
    lqr_outer_Y.motor_speed[i] = psi;
   ...
   *  MATLAB Function: '<Root>/MATLAB Function'
   */
  lqr_outer_Y.psi_ref = psi;

  /* Gain: '<S3>/ ' incorporates:
   *  Integrator: '<S3>/Integrator'
   *  Sum: '<S3>/Sum'
   */
  tmp_1[0] = 0.11180339887498918 * lqr_outer_X.Integrator_CSTATE_b[0] + -0.0 *
    lqr_outer_X.Integrator_CSTATE_b[1];
  tmp_1[1] = -0.0 * lqr_outer_X.Integrator_CSTATE_b[0];
  tmp_1[1] += 0.1118033988749903 * lqr_outer_X.Integrator_CSTATE_b[1];
  for (i = 0; i < 2; i++) {
    /* Gain: '<S3>/                   ' incorporates:
     *  Inport: '<Root>/X'
     ...
     *  Sum: '<S3>/Sum'
     */
    rtb_rads_to_RPM = lqr_outer_ConstP._Gain_a[i + 6] * lqr_outer_U.X[4] +
