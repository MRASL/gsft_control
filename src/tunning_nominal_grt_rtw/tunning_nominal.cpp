...
void tunning_nominalModelClass::step()
{
...

  {
     ...
     *  Sum: '<S10>/Sum1'
     */
    rtb_uNm_p = ((tunning_nominal_U.gain[8] *
      ...
      (1.0 / tunning_nominal_B.Product);

    /* Gain: '<Root>/Gain' */
    rtb_T_body = 1.54 * rtb_uNm_p;
    ...
    if (rtmIsMajorTimeStep((&tunning_nominal_M)) &&
       ...
       *  Fcn: '<Root>/Fcn1'
       */
      tunning_nominal_B.Gain1 = (tunning_nominal_B.T_outer[1] * std::cos
        (tunning_nominal_B.T_outer[2]) + tunning_nominal_B.T_outer[0] * std::sin
        (tunning_nominal_B.T_outer[2])) * 9.81;
    }
     ...
     */
    rtb_uNm_h = 1.0 / rtb_uNm_p * tunning_nominal_B.Gain1 -
    ...
    if (rtmIsMajorTimeStep((&tunning_nominal_M)) &&
       ...
       *  Fcn: '<Root>/Fcn'
       */
      tunning_nominal_B.Gain2 = (-tunning_nominal_B.T_outer[1] * std::sin
     ...
     */
    rtb_ref_idx_2 = 1.0 / rtb_uNm_p * tunning_nominal_B.Gain2 -
    ...
    /* Clock: '<Root>/Clock' */
    rtb_uNm_p = (&tunning_nominal_M)->Timing.t[0];
    ...
    for (i = 0; i < 6; i++) {
      ...
      u0 = tunning_nominal_ConstP.ControlAllocation_Gain[i + 18] * rtb_ref_idx_3
        + (tunning_nominal_ConstP.ControlAllocation_Gain[i + 12] * rtb_ref_idx_2
           + (tunning_nominal_ConstP.ControlAllocation_Gain[i + 6] * rtb_uNm_h +
              tunning_nominal_ConstP.ControlAllocation_Gain[i] * rtb_T_body));
      ...
      if (rtb_uNm_p >= tunning_nominal_U.LOE_t[i]) {
     ...
     *  SignalConversion: '<Root>/TmpSignal ConversionAtControl AllocationInport1'
     */
    tunning_nominal_Y.virtual_control[0] = rtb_T_body;
    tunning_nominal_Y.virtual_control[1] = rtb_uNm_h;
    ...
    switch ((int32_T)tunning_nominal_U.mode) {
     case 0:
      ...
      rtb_T_body = tunning_nominal_U.ref[0];
      rtb_uNm_p = tunning_nominal_U.ref[1];
     ...
     case 1:
      ...
      if (rtb_uNm_p <= 15.0) {
        ...
        rtb_T_body = tunning_nominal_U.Y0[0];
        rtb_uNm_p = tunning_nominal_U.Y0[1];
        rtb_ref_idx_2 = 1.0;
        rtb_ref_idx_3 = tunning_nominal_U.Y0[3];
      } else if (rtb_uNm_p <= 25.0) {
        /* '<S62>:1:9' elseif t <= 25 */
        /* '<S62>:1:10' ref = [Y0(1)+1; Y0(2)-1; 1; Y0(4)]; */
        rtb_T_body = tunning_nominal_U.Y0[0] + 1.0;
        rtb_uNm_p = tunning_nominal_U.Y0[1] - 1.0;
        rtb_ref_idx_2 = 1.0;
        rtb_ref_idx_3 = tunning_nominal_U.Y0[3];
      } else if (rtb_uNm_p <= 35.0) {
     ...
     case 2:
      ...
      if (rtb_uNm_p <= 10.0) {
        ...
        rtb_ref_idx_2 = 0.75;
        rtb_ref_idx_3 = tunning_nominal_U.Y0[3];
      } else if (rtb_uNm_p <= 50.0) {
        /* '<S62>:1:34' elseif t <= 50 */
        /* '<S62>:1:35' ref = [Y0(1)+cos(t); Y0(2)+sin(t); 0.75; Y0(4)]; */
        rtb_T_body = tunning_nominal_U.Y0[0] + std::cos(rtb_uNm_p);
        rtb_uNm_p = tunning_nominal_U.Y0[1] + std::sin(rtb_uNm_p);
        rtb_ref_idx_2 = 0.75;
        rtb_ref_idx_3 = tunning_nominal_U.Y0[3];
      } else if (rtb_uNm_p <= 60.0) {
    ...
    tunning_nominal_Y.ref_out[0] = rtb_T_body;
    tunning_nominal_Y.ref_out[1] = rtb_uNm_p;
    ...
    /* Sum: '<S6>/Sum1' */
    tunning_nominal_B.Sum1 = rtb_T_body - rtb_d_x;
    ...
    /* Sum: '<S6>/Sum4' */
    tunning_nominal_B.Sum4 = rtb_uNm_p - rtb_d_y;
    ...
    /* Sum: '<S9>/Sum3' */
    tunning_nominal_B.Sum3_n = rtb_ref_idx_3 - rtb_d_psi;
