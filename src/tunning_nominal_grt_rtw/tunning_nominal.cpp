...

  {
    ...
    if (tunning_nominal_U.mode == 2.0) {
      ...
      if ((rtb_Clock >= 10.0) && (rtb_Clock <= 50.0)) {
        ...
        /*   */
        /* '<S60>:1:14' ff = [axref_b; ayref_b; azref_b]; */
     ...
     *  Sum: '<S10>/Sum1'
     */
    axref_N = ((tunning_nominal_U.gain[8] * tunning_nominal_X.Integrator1_CSTATE
                - (tunning_nominal_U.gain[6] * rtb_d_z + tunning_nominal_U.gain
                   [7] * tunning_nominal_U.X[5])) * 0.64935064935064934 + 9.81) *
      (1.0 / tunning_nominal_B.Product) + rtb_ff[2];

    /* Gain: '<Root>/Gain' */
    ayref_N = 1.54 * axref_N;
     ...
     *  Sum: '<Root>/Sum7'
     */
    rtb_uNm_i = 1.0 / axref_N * (tunning_nominal_B.Gain1 - rtb_ff[1]) -
     ...
     *  Sum: '<Root>/Sum8'
     */
    axref_N = 1.0 / axref_N * (tunning_nominal_B.Gain2 + rtb_ff[0]) -
    ...
    for (i = 0; i < 6; i++) {
      ...
      u0 = tunning_nominal_ConstP.ControlAllocation_Gain[i + 18] * rtb_ref_idx_3
        + (tunning_nominal_ConstP.ControlAllocation_Gain[i + 12] * axref_N +
           (tunning_nominal_ConstP.ControlAllocation_Gain[i + 6] * rtb_uNm_i +
            tunning_nominal_ConstP.ControlAllocation_Gain[i] * ayref_N));
     ...
     *  SignalConversion: '<Root>/TmpSignal ConversionAtControl AllocationInport1'
     */
    tunning_nominal_Y.virtual_control[0] = ayref_N;
    tunning_nominal_Y.virtual_control[1] = rtb_uNm_i;
    tunning_nominal_Y.virtual_control[2] = axref_N;
