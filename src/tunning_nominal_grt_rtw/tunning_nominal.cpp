    ...
    if (tunning_nominal_U.mode == 2.0) {
      ...
      if ((rtb_Clock >= 10.0) && (rtb_Clock <= 50.0)) {
        ...
        rtb_ff[2] = (std::cos(tunning_nominal_U.X[6]) * std::sin
          ...
          cos(tunning_nominal_U.X[6]) * std::sin(tunning_nominal_U.X[7]) * std::
          sin(tunning_nominal_U.X[8]) - std::sin(tunning_nominal_U.X[6]) * std::
          cos(tunning_nominal_U.X[8])) * -std::sin(rtb_Clock);
      } else {
        /* '<S60>:1:15' else */
        /* '<S60>:1:16' ff = [0;0;0]; */
        rtb_ff[2] = 0.0;
