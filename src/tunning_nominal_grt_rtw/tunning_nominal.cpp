 ...
 * C++ source code generated on : Wed Aug 22 15:36:49 2018
 *
...
void tunning_nominalModelClass::step()
{
...

  {
    ...
    /* Clock: '<Root>/Clock' */
    rtb_Sum1_g = (&tunning_nominal_M)->Timing.t[0];
    ...
    if (tunning_nominal_U.mode == 2.0) {
      ...
      if ((rtb_Sum1_g >= 10.0) && (rtb_Sum1_g <= 50.0)) {
        /* '<S61>:1:7' axref_N = -cos(t); */
        rtb_Gain_a = -std::cos(rtb_Sum1_g);

        /* '<S61>:1:8' ayref_N = -sin(t); */
        zbody_d = -std::sin(rtb_Sum1_g);
     ...
     *  Sum: '<Root>/Sum2'
     */
    rtb_Gain1 = (tunning_nominal_B.att[0] + rtb_ff[2]) * 1.54;
     ...
     *  Sum: '<S9>/Sum1'
     */
    zbody_d = (tunning_nominal_B.Fcn1 + rtb_ff[0]) - (tunning_nominal_U.gain[9] *
    ...
    for (i = 0; i < 6; i++) {
      ...
      if (rtb_Sum1_g >= tunning_nominal_U.LOE_t[i]) {
    ...
    switch ((int32_T)tunning_nominal_U.mode) {
     ...
     case 2:
      ...
      if (rtb_Sum1_g <= 10.0) {
        ...
        rtb_Gain1 = tunning_nominal_U.Y0[0] + std::cos(rtb_Sum1_g);
        zbody_d = tunning_nominal_U.Y0[1] + std::sin(rtb_Sum1_g);
        rtb_ref_idx_2 = 0.75;
        rtb_ref_idx_3 = tunning_nominal_U.Y0[3];
      } else if (rtb_Sum1_g <= 60.0) {
    ...
    zbody_d -= tunning_nominal_U.Y0[1];
    rtb_ref_idx_2 -= tunning_nominal_U.Y0[2];
    rtb_Sum1_g = rtb_ref_idx_3 - tunning_nominal_U.Y0[3];
    ...
    /* Saturate: '<S10>/yaw' */
    if (rtb_Sum1_g > 3.1415926535897931) {
      rtb_Sum1_g = 3.1415926535897931;
    } else {
      if (rtb_Sum1_g < -3.1415926535897931) {
        rtb_Sum1_g = -3.1415926535897931;
      }
    ...
    /* Sum: '<S10>/Sum3' */
    tunning_nominal_B.Sum3 = rtb_Sum1_g - rtb_Gain_a;
