...
lqr_innerModelClass gController;
...
void OdometryCallback(const nav_msgs::Odometry::ConstPtr &odom) {
  ...
  gController.lqr_inner_U.X[0 ]  = odometry.position_W.x();
  gController.lqr_inner_U.X[1 ]  = odometry.position_W.y();
  gController.lqr_inner_U.X[2 ]  = odometry.position_W.z();

  Eigen::Matrix3d R_W_B = odometry.orientation_W_B.toRotationMatrix();
  Eigen::Vector3d velocity_W =  R_W_B * odometry.velocity_B;

  gController.lqr_inner_U.X[3 ]  = velocity_W.x();
  gController.lqr_inner_U.X[4 ]  = velocity_W.y();
  gController.lqr_inner_U.X[5 ]  = velocity_W.z();
  ...
  gController.lqr_inner_U.X[6 ]  = phi;
  gController.lqr_inner_U.X[7 ]  = teta;
  gController.lqr_inner_U.X[8 ]  = psi;
  ...
  gController.lqr_inner_U.X[9 ]  = euler_rate.x();
  gController.lqr_inner_U.X[10]  = euler_rate.y();
  gController.lqr_inner_U.X[11]  = euler_rate.z();
}
...
/*void MultiDofJointTrajectoryCallback(
    ...
    gController.lqr_inner_U.z_ref    = eigen_reference.position_W.z();
...
/*void CommandPoseCallback(const geometry_msgs::PoseStampedConstPtr& pose_msg) {
  ...
  gController.lqr_inner_U.x_ref    = eigen_reference.position_W.x();
  gController.lqr_inner_U.y_ref    = eigen_reference.position_W.y();
  gController.lqr_inner_U.z_ref    = eigen_reference.position_W.z();
  gController.lqr_inner_U.psi_ref  = 0.0;
  if (!gCommand_active){
    gCommand_active      = true;
  }
}
...
int main(int argc, char** argv) {
  ros::init(argc, argv, "lqr_inner_controller_node");
  ros::NodeHandle nh;
  ros::NodeHandle pnh("~");
  ROS_INFO("lqr_inner_controller_node main started");
...
/*  ros::Subscriber cmd_pose_sub_;                    // references (position and yaw)
  ...
  while(ros::ok()) {
    if (gCommand_active) {
        ...
        for(unsigned int i=0; i< 6; i++) {
          if (gEmergency_status)
          {
            ...
            motor_RPM[i]     = gController.lqr_inner_Y.motor_RPM[i]*(1.0 - gLOE[i]);
            motor_command[i] = gController.lqr_inner_Y.motor_command[i]*(1.0 - gLOE[i]);
            motor_speed[i]   = gController.lqr_inner_Y.motor_speed[i]*(1.0 - gLOE[i]);
          }
        }
    ...
    uav_state_msg->position_ref.x  = gController.lqr_inner_Y.ref[0];
    uav_state_msg->position_ref.y  = gController.lqr_inner_Y.ref[1];
    uav_state_msg->position_ref.z  = gController.lqr_inner_Y.ref[2];
    uav_state_msg->heading_ref     = gController.lqr_inner_Y.ref[3];

    uav_state_msg->position_W.x  = gController.lqr_inner_U.X[0];
    uav_state_msg->position_W.y  = gController.lqr_inner_U.X[1];
    uav_state_msg->position_W.z  = gController.lqr_inner_U.X[2];

    uav_state_msg->velocity_W.x  = gController.lqr_inner_U.X[3];
    uav_state_msg->velocity_W.y  = gController.lqr_inner_U.X[4];
    uav_state_msg->velocity_W.z  = gController.lqr_inner_U.X[5];

    uav_state_msg->euler_angle.x = gController.lqr_inner_U.X[6];
    uav_state_msg->euler_angle.y = gController.lqr_inner_U.X[7];
    uav_state_msg->euler_angle.z = gController.lqr_inner_U.X[8];

    uav_state_msg->euler_rate.x  = gController.lqr_inner_U.X[9];
    uav_state_msg->euler_rate.y  = gController.lqr_inner_U.X[10];
    uav_state_msg->euler_rate.z  = gController.lqr_inner_U.X[11];

    uav_state_msg->total_thrust  = gController.lqr_inner_Y.virtual_control[0];
    uav_state_msg->moment.x      = gController.lqr_inner_Y.virtual_control[1];
    uav_state_msg->moment.y      = gController.lqr_inner_Y.virtual_control[2];
    uav_state_msg->moment.z      = gController.lqr_inner_Y.virtual_control[3];
