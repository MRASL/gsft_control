 ...
 * C++ source code generated on : Mon Jan 29 21:24:55 2018
 *
...
void bsl_hoveringModelClass::step()
{
...

  {
     ...
     */
    rtb_Sum1_j = 0.12909944487357924 * bsl_hovering_X.Integrator1_CSTATE_d -
      (0.26326488685185462 * bsl_hovering_U.X[0] + 0.23615542024792072 *
       bsl_hovering_U.X[3]);
     ...
     */
    rtb_Sum1_b = -0.12909944487357847 * bsl_hovering_X.Integrator1_CSTATE_j -
      (-0.25521022183356207 * bsl_hovering_U.X[1] + -0.21998128671514852 *
       bsl_hovering_U.X[4]);

    /* Fcn: '<Root>/Fcn1' incorporates:
     *  Inport: '<Root>/X'
     */
    u0 = rtb_Sum1_b * std::cos(bsl_hovering_U.X[8]) + rtb_Sum1_j * std::sin
      (bsl_hovering_U.X[8]);
     ...
     */
    rtb_Sum3_g = u0 - (1.0339605451100693 * bsl_hovering_U.X[6] +
                       0.26871289347806038 * bsl_hovering_U.X[9]);
     ...
     *  Sum: '<S8>/Sum1'
     */
    rtb_Sum1_b = 8.660254037844453 * bsl_hovering_X.Integrator1_CSTATE -
      (11.352186290279159 * bsl_hovering_U.X[2] + 5.9970604277479032 *
       bsl_hovering_U.X[5]);
     ...
     *  Sum: '<S5>/Sum1'
     */
    rtb_Sum1_j -= 1.1688818206856761 * bsl_hovering_U.X[7] + 0.32805500506839508
      * bsl_hovering_U.X[10];
    tmp_4 = 0.12909944487358019 * bsl_hovering_X.Integrator1_CSTATE_a -
      (0.26766133738120873 * bsl_hovering_U.X[8] + 0.22905827204219203 *
       bsl_hovering_U.X[11]);
    ...
    bsl_hovering_Y.ref[3] = rtb_Sum1_b;
    if (rtmIsMajorTimeStep((&bsl_hovering_M))) {
      /* UnitDelay: '<S12>/       ' */
      for (i = 0; i < 6; i++) {
      ...
      for (x = 0; x < 12; x++) {
        ...
        for (i = 0; i < 12; i++) {
          ...
          u0 = 0.0;
          for (i_0 = 0; i_0 < 12; i_0++) {
            u0 += rtb_Product3[12 * i_0 + x] * bsl_hovering_ConstP.A_Gain[12 * i
              + i_0];
          }

          rtb_Sum4[x + 12 * i] = ((rtb_Product2_e_0[12 * i + x] - rtb_Divide_1
            [12 * i + x]) + u0) + bsl_hovering_ConstP.Q_x_Value[12 * i + x];
      ...
      for (x = 0; x < 6; x++) {
        for (i = 0; i < 6; i++) {
          ...
          u0 = 0.0;
          for (i_0 = 0; i_0 < 12; i_0++) {
            u0 += bsl_hovering_ConstP.pooled4[6 * i_0 + i] * rtb_Sum4_0[12 * x +
              i_0];
          }

          tmp[i + 6 * x] = bsl_hovering_ConstP.pooled7[6 * x + i] + u0;
      ...
      for (x = 0; x < 12; x++) {
        ...
        for (i = 0; i < 6; i++) {
           ...
           *  Product: '<S9>/Product'
           */
          u0 += rtb_Product2_o[12 * i + x] * bsl_hovering_B.gamma_hat_k1k[i];
        ...
        bsl_hovering_B.X_tildle_k1k[x] = ((tmp_0[x] + tmp_1[x]) +
          bsl_hovering_ConstP.Constant_Value_g[x]) + u0;
      }
    ...
    for (i = 0; i < 6; i++) {
      ...
      u0 = 0.0;
      for (x = 0; x < 6; x++) {
        u0 += bsl_hovering_B.K_gamma_k1[6 * x + i] * tmp_3[x];
      }

      bsl_hovering_B.Sum1[i] = bsl_hovering_B.gamma_hat_k1k[i] + u0;
    ...
    if (rtmIsMajorTimeStep((&bsl_hovering_M))) {
      ...
      for (x = 0; x < 6; x++) {
        for (i = 0; i < 6; i++) {
          u0 = 0.0;
          for (i_0 = 0; i_0 < 12; i_0++) {
            u0 += bsl_hovering_ConstP.pooled4[6 * i_0 + i] * rtb_Product2_o[12 *
              x + i_0];
          }
    ...
    for (x = 0; x < 12; x++) {
      u0 = 0.0;
      for (i = 0; i < 6; i++) {
        u0 += bsl_hovering_B.K_tildle_x_k1[12 * i + x] * tmp_3[i];
      }

      bsl_hovering_B.Sum3[x] = bsl_hovering_B.X_tildle_k1k[x] + u0;
    }
    ...
    if (rtmIsMajorTimeStep((&bsl_hovering_M))) {
      ...
      for (x = 0; x < 6; x++) {
        ...
        for (i = 0; i < 12; i++) {
          u0 = 0.0;
          for (i_0 = 0; i_0 < 6; i_0++) {
            u0 += bsl_hovering_B.K_tildle_x_k1[12 * i_0 + i] * rtb_H_k1k[6 * x +
              i_0];
          }

          rtb_Sum1_a[i + 12 * x] = rtb_Divide[12 * x + i] - u0;
        }
      ...
      for (x = 0; x < 6; x++) {
        for (i = 0; i < 6; i++) {
          u0 = 0.0;
          for (i_0 = 0; i_0 < 6; i_0++) {
            u0 += bsl_hovering_B.K_gamma_k1[6 * i_0 + i] * rtb_H_k1k_0[6 * x +
              i_0];
          }

          rtb_P_gamma_k1[i + 6 * x] = rtb_P_gamma_k1k[6 * x + i] - u0;
        }
      }
     ...
     *  MATLAB Function: '<Root>/MATLAB Function'
     */
    u0 = rtb_Sum1_b - bsl_hovering_U.X[8];
     ...
     *  Saturate: '<S6>/x'
     */
    u0 = (real_T)(int32_T)rtb_Sum3_g - bsl_hovering_U.X[0];
     ...
     *  MATLAB Function: '<Root>/MATLAB Function'
     */
    u0 = (real_T)y - bsl_hovering_U.X[1];
     ...
     *  MATLAB Function: '<Root>/MATLAB Function'
     */
    u0 = rtb_Sum1_j - bsl_hovering_U.X[2];
