...
lqr_outerModelClass gController;
...
void OdometryCallback(const nav_msgs::Odometry::ConstPtr &odom) {
  ...
  gController.lqr_outer_U.X[0 ]  = odometry.position_W.x();
  gController.lqr_outer_U.X[1 ]  = odometry.position_W.y();
  gController.lqr_outer_U.X[2 ]  = odometry.position_W.z();

  Eigen::Matrix3d R_W_B = odometry.orientation_W_B.toRotationMatrix();
  Eigen::Vector3d velocity_W =  R_W_B * odometry.velocity_B;

  gController.lqr_outer_U.X[3 ]  = velocity_W.x();
  gController.lqr_outer_U.X[4 ]  = velocity_W.y();
  gController.lqr_outer_U.X[5 ]  = velocity_W.z();
  ...
  gController.lqr_outer_U.X[6 ]  = phi;
  gController.lqr_outer_U.X[7 ]  = teta;
  gController.lqr_outer_U.X[8 ]  = psi;
...
/*void MultiDofJointTrajectoryCallback(
    ...
    gController.lqr_outer_U.z_ref    = eigen_reference.position_W.z();
...
/*void CommandPoseCallback(const geometry_msgs::PoseStampedConstPtr& pose_msg) {
  ...
  gController.lqr_outer_U.x_ref    = eigen_reference.position_W.x();
  gController.lqr_outer_U.y_ref    = eigen_reference.position_W.y();
  gController.lqr_outer_U.z_ref    = eigen_reference.position_W.z();
  gController.lqr_outer_U.psi_ref  = 0.0;
  if (!gCommand_active){
    gCommand_active      = true;
  }
}
...
int main(int argc, char** argv) {
  ros::init(argc, argv, "lqr_outer_controller_node");
  ros::NodeHandle nh;
  ros::NodeHandle pnh("~");
  ROS_INFO("lqr_outer_controller_node main started");
...
/*  ros::Subscriber cmd_pose_sub_;                    // references (position and yaw)
  ...
  while(ros::ok()) {
    if (gCommand_active) {
        ...
        for(unsigned int i=0; i< 6; i++) {
          if (gEmergency_status)
          {
            ...
            motor_RPM[i]     = gController.lqr_outer_Y.motor_RPM[i]*(1.0 - gLOE[i]);
            motor_command[i] = gController.lqr_outer_Y.motor_command[i]*(1.0 - gLOE[i]);
            motor_speed[i]   = gController.lqr_outer_Y.motor_speed[i]*(1.0 - gLOE[i]);
          }
        }
    ...
    uav_state_msg->position_ref.x  = gController.lqr_outer_Y.x_ref;
    uav_state_msg->position_ref.y  = gController.lqr_outer_Y.y_ref;
    uav_state_msg->position_ref.z  = gController.lqr_outer_Y.z_ref;
    ...
    uav_state_msg->moment.y      = gController.lqr_outer_Y.virtual_control[2];
    uav_state_msg->moment.z      = gController.lqr_outer_Y.virtual_control[3];
