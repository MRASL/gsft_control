  ...
  while(ros::ok()) {
    if (gCommand_active) {
        ...
        for(unsigned int i=0; i< 6; i++) {
            motor_RPM[i]     = gController.lqr_outer_Y.motor_RPM[i];
            motor_command[i] = gController.lqr_outer_Y.motor_command[i];
            motor_speed[i]   = gController.lqr_outer_Y.motor_speed[i];
        ...
        virtual_contrl_msg->total_thrust = gController.lqr_outer_Y.virtual_control[0];

        Eigen::VectorXd moment(3);
        for(unsigned int i=0; i< 2; i++)
            moment[i] = gController.lqr_outer_Y.virtual_control[i+1];
        moment[2] = 0.0;    // yaw
