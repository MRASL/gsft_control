 ...
 * C++ source code generated on : Fri Feb  9 14:58:56 2018
 *
...
void scenario1_lqrModelClass::step()
{
...

  {
     ...
     */
    rtb_ixj = 0.09999999999999809 * scenario1_lqr_X.Integrator1_CSTATE_h -
      (0.20466962988047713 * rtb_VectorConcatenate[0] + 0.18944828697705904 *
     ...
     */
    rtb_kxi = -0.12909944487358244 * scenario1_lqr_X.Integrator1_CSTATE_j -
      (-0.23954240432861679 * rtb_VectorConcatenate[1] + -0.2007173925756503 *
       rtb_VectorConcatenate[4]);

    /* Fcn: '<Root>/Fcn1' incorporates:
     *  Inport: '<Root>/X'
     */
    rtb_jxk = rtb_kxi * std::cos(scenario1_lqr_U.X[8]) + rtb_ixj * std::sin
      (scenario1_lqr_U.X[8]);
     ...
     */
    rtb_jxk -= 0.95248094519569515 * rtb_VectorConcatenate[6] +
      0.25774337852451673 * rtb_VectorConcatenate[9];

    /* Fcn: '<Root>/Fcn' incorporates:
     *  Inport: '<Root>/X'
     */
    rtb_ixj = -rtb_kxi * std::sin(scenario1_lqr_U.X[8]) + rtb_ixj * std::cos
      (scenario1_lqr_U.X[8]);
     ...
     *  Sum: '<S9>/Sum1'
     */
    rtb_ref_idx_0 = 10.000000000000028 * scenario1_lqr_X.Integrator1_CSTATE -
      (12.219481695651963 * rtb_VectorConcatenate[2] + 6.2157866455186568 *
     ...
     *  Sum: '<S6>/Sum1'
     */
    rtb_ixj -= 0.99863446436101266 * rtb_VectorConcatenate[7] +
      0.30308491090608103 * rtb_VectorConcatenate[10];
    rtb_kxi = 0.0816496580927725 * scenario1_lqr_X.Integrator1_CSTATE_b -
      (0.20714107651319064 * rtb_VectorConcatenate[8] + 0.20151600354316992 *
    ...
    for (i = 0; i < 6; i++) {
       ...
       *  Sum: '<Root>/Sum2'
       */
      scenario1_lqr_B.u[i] = 0.0;
      scenario1_lqr_B.u[i] += (rtb_ref_idx_0 + 15.107400000000002) *
        scenario1_lqr_ConstP._Gain_m[i];
      scenario1_lqr_B.u[i] += scenario1_lqr_ConstP._Gain_m[i + 6] * rtb_jxk;
      scenario1_lqr_B.u[i] += scenario1_lqr_ConstP._Gain_m[i + 12] * rtb_ixj;
      scenario1_lqr_B.u[i] += scenario1_lqr_ConstP._Gain_m[i + 18] * rtb_kxi;

      /* MATLAB Function: '<Root>/Actuator_Fault' */
      rtb_T_f[i] = scenario1_lqr_B.u[i];
    }
    ...
    if (rtb_Clock >= 20.0) {
      ...
      rtb_T_f[0] = scenario1_lqr_B.u[0] * 0.8;
      rtb_T_f[1] = scenario1_lqr_B.u[1] * 0.8;
      rtb_T_f[2] = scenario1_lqr_B.u[2] * 0.8;
      rtb_T_f[3] = scenario1_lqr_B.u[3] * 0.8;
      rtb_T_f[4] = scenario1_lqr_B.u[4] * 0.8;
      rtb_T_f[5] = scenario1_lqr_B.u[5] * 0.8;
    }

    /* Outport: '<Root>/virtual_control' incorporates:
     *  Sum: '<Root>/Sum2'
     */
    scenario1_lqr_Y.virtual_control[0] = rtb_ref_idx_0 + 15.107400000000002;
    scenario1_lqr_Y.virtual_control[1] = rtb_jxk;
    scenario1_lqr_Y.virtual_control[2] = rtb_ixj;
    scenario1_lqr_Y.virtual_control[3] = rtb_kxi;
    for (i = 0; i < 6; i++) {
      /* Sqrt: '<Root>/Sqrt1' incorporates:
       *  Gain: '<Root>/      '
       */
      rtb_ref_idx_0 = std::sqrt(149253.73134328358 * rtb_T_f[i]);
      ...
      if (rtb_jxk > 200.0) {
        ...
        /* Outport: '<Root>/motor_command' */
        scenario1_lqr_Y.motor_command[i] = rtb_jxk;
      }

      /* End of Saturate: '<Root>/Saturation' */

      /* Outport: '<Root>/motor_speed' */
      scenario1_lqr_Y.motor_speed[i] = rtb_ref_idx_0;

      /* Outport: '<Root>/motor_RPM' */
      scenario1_lqr_Y.motor_RPM[i] = rtb_ixj;
    }
    ...
    if (rtb_Clock <= 40.0) {
      ...
      /* '<S3>:1:5' y = X0(2); */
      rtb_ixj = scenario1_lqr_U.X0[1];

      /* '<S3>:1:6' z = 0.75; */
      rtb_kxi = 0.75;

      /* '<S3>:1:7' psi = X0(4); */
      rtb_jxk = scenario1_lqr_U.X0[3];
    } else {
      /* '<S3>:1:8' else */
      /* '<S3>:1:9' x = X0(1); */
      rtb_Clock = scenario1_lqr_U.X0[0];
    ...
    scenario1_lqr_Y.ref[1] = rtb_ixj;
    scenario1_lqr_Y.ref[2] = rtb_kxi;
    scenario1_lqr_Y.ref[3] = rtb_jxk;
    if (rtmIsMajorTimeStep((&scenario1_lqr_M))) {
      /* Memory: '<S2>/Memory' */
      for (i = 0; i < 6; i++) {
     ...
     *  MATLAB Function: '<Root>/MATLAB Function'
     */
    rtb_ref_idx_0 -= scenario1_lqr_U.X0[0];
    rtb_Clock = rtb_ixj - scenario1_lqr_U.X0[1];
    rtb_ixj = rtb_kxi - scenario1_lqr_U.X0[2];
    rtb_jxk -= scenario1_lqr_U.X0[3];
  ...
  if (rtmIsMajorTimeStep((&scenario1_lqr_M))) {
    ...
    if (rtmIsMajorTimeStep((&scenario1_lqr_M))) {
      ...
      for (i = 0; i < 6; i++) {
        scenario1_lqr_DW.Memory_PreviousInput[i] = scenario1_lqr_B.u[i];
      }
...
void scenario1_lqrModelClass::initialize()
{
  ...
  (&scenario1_lqr_M)->Timing.stepSize0 = 0.005;
